...
analysis_data <- data %>%
  ...
  mutate(match_true=case_when((responses==true_plot )~"Yes",
...
reference_plot <- analysis_data %>%
  filter((plot_type=="control")&(match_true=="No"))%>%
...
analysis_data %>%
count(version,match_true)

analysis_data%>%
  ggplot(aes(x=version,fill=match_true))+
  geom_bar()

analysis_data%>%
  count(resid,plot_type,version,match_true)%>%
...
analysis_data%>%
 ...
 # filter(resid=="Least-confounded")%>%
  ggplot(aes(x=version,y=n,fill=match_true))+
...
```{r}
  ...
  analysis_data %>%
  group_by(resid,plot_type,version,match_true)%>%
  count(certainity)%>%
  mutate(
    pname=case_when(
  ...
  ggplot(aes(x=certainity,y=n,fill=match_true))+
  geom_col(position = "dodge")+
